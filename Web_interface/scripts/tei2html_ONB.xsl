        ...
        <main>
                <xsl:for-each select="/TEI/text/body/pb">
                   ...
                   <!-- <xsl:variable name="pbID" select="substring-after(@corresp, 'f')"/>-->
                    <xsl:variable name="pbID" select="substring-after(@corresp, 'f')"/>
                    <xsl:variable name="pbIDimg">
                        <xsl:variable name="part1" select="substring-before($pbID, '_')"/>
                        <xsl:variable name="rest"  select="substring-after($pbID, '_')"/>
                        <xsl:variable name="part2" select="substring-before($rest, '_')"/>
                        <xsl:choose>
                            <!-- onb.ac.at pattern: two alphanum segments separated by _ -->
                            <xsl:when test="contains($pbID, '_') and string-length($part2) > 0">
                                <xsl:value-of select="concat($part1, '/', $part2)"/>
                            </xsl:when>
                            <!-- e-rara.ch pattern: simple numeric ID, keep as-is -->
                            <xsl:otherwise>
                    ...
                    <div class="content-wrapper">
                        ...
                        <figure class="image-text">
                            <img src="{concat('https://api.onb.ac.at/iiif/image/v3/', $pbIDimg, '/full/full/0/default.jpg')}"
                                alt="{substring-after(@corresp, 'f')}"/>
                        </figure>
                     <!--   <figure class="image-text">  <!-\- div image-section => figure -\->
                            <img src="{concat('https://api.onb.ac.at/iiif/image/v3/', substring-after(@corresp, 'f'), '/full/full/0/default.jpg')}" alt="{substring-after(@corresp,'f')}"/>
                        </figure>-->
